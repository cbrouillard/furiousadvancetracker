 ...
 % Cette commande permet de sauter directement au block suivant. Elle peut permettre de créer des décalages de temps entre les blocks.
  ...
  Voyons la marche à suivre.\medskip
    ...
    \begin{itemize}
      ...
      \item{créez le fichier 00nbkit qui compte le nombre de kits dans la ROM finale (le nombre de gbfs donc)}
  ...
  Après toutes ces manipulations, votre ROM est prête : vos samples sont accessible sur les channels SNA et SNB avec des instruments de type SAMPLE. Enjoy !\medskip

  Notez, pour les utilisateurs de Linux, que gbfs.exe tourne très bien avec Wine.

  \section{(OBSOLETE) Backup de vos sauvegardes}\label{burger}

  \ColoredAnnotation{L'application de gestion de backup de sauvegardes va être mergée et integrée au sein du futur site web dédié à \FAT. Il sera possible de gérer vos sauvegardes et kits en ligne ! Cette partie de la documentation est donc obsolète et sera supprimée très bientôt.}
