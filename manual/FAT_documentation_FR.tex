...
\begin{document}
  ...
  \section{Présentation}
    ...
    \begin{itemize}
        \item{sauvergarde et chargement : 3 chansons (pas de compression) sont gérées}
    ...
    La GBA ne possède qu'un haut-parleur mono. Si vous souhaitez profiter des possibilités stéréo de \FAT, il vous faudra brancher un casque sur la sortie prévue à cet effet. La GBA ne sort du stéréo que via cette prise.
        ...
        \ColoredAnnotation{Vous pouvez utiliser ce canal générer la mélodie.}
        ...
        \ColoredAnnotation{Ce canal n'est pas dispo pour cette version de \FAT. Il pourra être utilisable pour lire les samples pré-embarqués dans la ROM}
  ...
  \section{Les écrans}
    ...
    \end{itemize}\medskip
     ...
     Mais \FAT est suffisamment souple pour permettre l'utilisation d'un instrument dans un canal qui ne lui est pas dédié: le comportement peut cependant être surprenant (voire carrément "hallucinant"). A vous de voir ce que vous souhaitez produire comme sonorités.
        ...
        \ColoredAnnotation{Ce type d'instrument n'est pas encore disponible. Dans une version future, il sera possible ici de créer des instruments branchés sur des collections de samples ou des instruments permettant de faire de la synthèse FM. La lecture des samples est prévue pour la version 0.3.0. Patience !}
    ...
    L'écran FILESYSTEM permet de sauvegarder et de charger les tracks sauvegardées sur la cartouche. Notez que cet écran dispose de deux modes : SAVE ou LOAD. Les deux modes sont visuellement très proches: les actions possibles sont néanmoins très différentes.
        ...
        \ColoredAnnotation{Pour cette version de \FAT, seuls 3 slots sont disponibles pour la sauvegarde. Les slots non disponibles sont marqués d'un "XXXXXX". Si vous essayez malgré tout d'enregistrer sur ces slots, une boîte de dialogue vous indiquera que votre action n'est pas valide.}        
        ...
        \ColoredAnnotation{Pour cette version de \FAT, seuls 3 slots sont disponibles pour le chargement. Les slots non disponibles sont marqués d'un "XXXXXX". Si vous essayez malgré tout de charger un de ces slots, une boîte de dialogue vous indiquera que votre action n'est pas valide.}  
