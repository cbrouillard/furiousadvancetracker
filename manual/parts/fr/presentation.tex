...
\ColoredAnnotation{
\WriteTextWithImgOnLeft{images/ezflash4}{0.1}{Si vous avez un souci de curseur "noir" avec l'ezFlash IV, voici une solution trouvée par \textcolor{bleu}{\bf @maz\_hoot} (merci pour le partage !)

\medskip

{\bf Procédure : Écrire FAT dans la mémoire NOR.}\\
(Ça marche sur l’EZ flash IV avec micro sd mais sur les anciennes version avec sd j’imagine que c’est pareil)
\medskip
\begin{enumerate}
  \item{allumer la console}
  \item{une fois l’écran d’accueil affiché, placer le curseur sur la planète et appuyer sur A 2 fois (c’est ce que vous faites d’habitude normalement).}
  \item{appuyer sur A pour entrer dans le dossier EZ-Disk (comme d’habitude)}
  \item{mettre le curseur sur FAT.gba}
  \item{appuyer sur select. Une boite de dialogue apparaît.}
  \item{appuyer sur A. FAT.gba s’écrit sur la cartouche en mode NOR. Ça peut prendre du temps c’est normal.}
  \item{vous devriez vous retrouver devant le dossier EZ-Disk sauf qu’il y aura FAT.gba juste en dessous. Si ce n'est pas le cas peut être qu’il faut éteindre et rallumer la console une fois que l’écriture est terminée.}
  \item{lancer FAT.gba en appuyant sur A depuis cet endroit là. Ça va se lancer beaucoup plus vite et il n'y aura plus ce problème de curseur noir.}
\end{enumerate}
}}
