...
\medskip Best choice is ezflashI.

\ColoredAnnotation{
\WriteTextWithImgOnLeft{images/ezflash4}{0.1}{If you have a "black" cursor issue, then try this solution brought to you by \textcolor{bleu}{\bf @maz\_hoot} (thanks for your share !)

\medskip

{\bf Howto : Write FAT on NOR memory.}\\

\medskip
\begin{enumerate}
  \item{switch on the console}
  \item{one the welcome screen displayed, place the cursor on planet icon and press A twice (as usual).}
  \item{press A to enter EZ-Disk directory}
  \item{set the cursor on FAT.gba}
  \item{press select. A dialog box appears.}
  \item{press A. FAT.gba is now writing with NOR mode on the cartridge. It may take some time to finish, please wait.}
  \item{now, you should see the EZ-Disk with FAT.gba below. If not please try to switch off and on your console.}
  \item{start FAT.gba by kicking the A button. It should be faster to start and cursors should not been black anymore.}
\end{enumerate}
}}
